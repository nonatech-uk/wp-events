<?php
/**
 * GitHub Plugin Updater for Events
 *
...
class Events_GitHub_Updater {
     ...
     * @param string $repo GitHub repository (e.g., 'nonatech-uk/wp-events')
    ...
    public function __construct($plugin_file, $repo, $current_version) {
        ...
        $this->cache_key = 'events_github_update_' . md5($repo);
    ...
    public function add_check_update_link($links) {
        $url = wp_nonce_url(admin_url('plugins.php?events_check_update=1'), 'events_check_update');
    ...
    public function handle_check_update() {
        if (empty($_GET['events_check_update'])) {
            return;
        }

        if (!current_user_can('update_plugins') || !wp_verify_nonce($_GET['_wpnonce'], 'events_check_update')) {
            return;
        }
